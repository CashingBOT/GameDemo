...
class WithdrawInfo extends Core.BaseModel {
    ...
    /** 小额提现 */
    // @Core.field
    // public smallWithdrawLevel: number = null;
    ...
    public init(data: xxgBuf.WithdrawalRet): void {
        this.redBagBalance = Number((data.redBagBalance / 100).toFixed(2));
...
export default class WithdrawLogic extends Core.ViewLogic {
    ...
    private updateModel(): void {
        ...
        this.lbBalance.string = "" + WithdrawData.redBagBalance;
    ...
    private async onClickWithdraw(): Promise<void> {
        ...
        if (this.withdrawAmount / 100 > WithdrawData.redBagBalance) {
            PromptService.prompt("红包余额不足");
            return;
        }
