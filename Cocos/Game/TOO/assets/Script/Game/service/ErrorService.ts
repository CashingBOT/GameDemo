...
class ErrorControl extends Core.BaseLogic {
     ...
     * ws连接没有建立
     */
    @Core.event(Core.CoreEventMap.CORE_WS_NOT_CONNECTION)
    private wsNotConnection() {
        Core.webSockets.close();
        GameCore.openPopup({
            type: PopupType.HINT,
            msg: `网络异常，请重新登录!`,
     ...
     *摊位等级不够 至尊体验时
     */
    @Core.code(CodeMap.STALL_ERROR_6207)
    private code_6207(code) {
        GameCore.openPopup({
            type: PopupType.HINT,
            msg: `摊位等级不够(${code})`,
            confirm: () => {
     ...
     * 账号被冻结
     */
    @Core.code(CodeMap.CODE_4905)
    private code_4905(code) {
        GameCore.openPopup({
            type: PopupType.HINT,
            msg: `该账号已被冻结！(${code})`,
            closeButtonShow: false,
     ...
     * 账号被冻结
     */
    @Core.code(CodeMap.CODE_1002)
    private code_1002(code) {
        GameCore.openPopup({
            type: PopupType.HINT,
            msg: `该账号已被冻结！(${code})`,
            closeButtonShow: false,
     ...
     * 请求超时
     */
    @Core.code(Core.CoreCodeMap.CORE_HTTP_REQUEST_TIMEOUT)
     ...
     * 服务器发生异常
     */
    @Core.code(Core.CoreCodeMap.CORE_HTTP_SERVER_ABNORMAL)
    private httpServerAbnormal(code: number) {
        GameCore.openPopup({
            type: PopupType.HINT,
            msg: `服务器发生异常（${code}）`,
            closeButtonShow: false,
     ...
     * 游戏维护踢下线
     */
    @Core.code(CodeMap.GAME_MAINTAIN)
    private gameMaintain(code: number) {
        GameCore.openPopup({
            type: PopupType.HINT,
            msg: `游戏正在维护，请稍后再试！（${code}）`,
            closeButtonShow: false,
            confirm: () => {
                Core.CommonFun.reboot();
            },
